...
class SheetsService {
  constructor() {
    ...
    if (!ipcRenderer) {
      throw new Error('Electron ipcRenderer is not available. Run via Electron.');
    }
  ...
  async readSheet(sheetName, range = 'A:Z') {
    try {
      const values = await ipcRenderer.invoke('sheets:read', {
        sheetId: this.sheetId,
        sheetName,
        range
      });
      return this.parseSheetData(values);
    } catch (error) {
      throw error;
    }
  }
  ...
  async writeSheet(sheetName, values, range = 'A1') {
    try {
      const data = await ipcRenderer.invoke('sheets:write', {
        sheetId: this.sheetId,
        sheetName,
        range,
        values
  ...
  async appendToSheet(sheetName, values) {
    try {
      const data = await ipcRenderer.invoke('sheets:append', {
        sheetId: this.sheetId,
        sheetName,
        values
      });
  ...
  async getAllPlayerStatsData() {
    try {
      ...
      // Check if all data is available in cache (CacheManager handles this)
