// PKS Service for Web (HTTP API)
// This service handles PKS data via HTTP API for web browsers
...
class PKSServiceWeb {
  ...
  async getPKSData() {
    try {
      ...
      // Read from Google Sheets via API (PKS specific endpoint)
      const response = await fetch(`${this.apiBaseUrl}/pks/read`, {
        method: 'POST',
        headers: {
      ...
      if (!result.success) {
        throw new Error(result.error || 'Failed to read PKS data');
      }

      const data = this.parseSheetData(result.data);
...
export default new PKSServiceWeb();
