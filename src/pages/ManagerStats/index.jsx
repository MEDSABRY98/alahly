...
export default function ManagerStats() {
  ...
  // Manager selection and active tab
  const [selectedManager, setSelectedManager] = useState('');
  const [activeTab, setActiveTab] = useState('competitions'); // competitions, seasons, opponents, opponent-managers, matches
  
  // Unique values for dropdowns
  const [uniqueValues, setUniqueValues] = useState({});
  
  // State persistence key
  const STATE_KEY = 'managerStatsPageState';
  ...
  }, [filterSyscom, filterChampion, filterSeason, filterRef, filterManagerOpp, filterHAN, filterStad, filterTeamOpp, filterWDL, dateFrom, dateTo, selectedManager, activeTab]);
  ...
  const loadState = useCallback(() => {
    try {
      ...
      if (savedState) {
        ...
        if (state.filterRef !== undefined) setFilterRef(state.filterRef);
        ...
        if (state.selectedManager !== undefined) setSelectedManager(state.selectedManager);
      ...
      console.error('Error loading ManagerStats state:', error);
    }
    return false;
  }, []);
  ...
  useEffect(() => {
    const loadManagerData = async () => {
      try {
        ...
        // Load player details data for goals calculation
        try {
  ...
  const getAvailableOptions = useCallback((column) => {
    ...
    // If no manager is selected, use cached unique values or all matches
    if (!selectedManager) {
    ...
    // If manager is selected, filter options based on manager's matches only
    const tempFilteredMatches = matches.filter(match => {
      // First filter by selected manager
      if (selectedManager && (!match['AHLY MANAGER'] || match['AHLY MANAGER'].toString().trim() !== selectedManager)) {
        return false;
      }
      ...
      return matchesSyscom && matchesChampion && matchesSeason && 
             matchesRef && matchesManagerOpp && matchesHAN && 
  ...
  }, [selectedManager, storeUniqueValues, matches, filterSyscom, filterChampion, filterSeason, filterRef, filterManagerOpp, filterHAN, filterStad, filterTeamOpp, filterWDL, dateFrom, dateTo]);
  ...
  const handleManagerSelection = (managerName) => {
    setSelectedManager(managerName);
    
    // Clear all filters when manager changes
    setFilterSyscom('');
    setFilterChampion('');
    setFilterSeason('');
    setFilterRef('');
  ...
  // Get all manager names from matches
  const allManagers = useMemo(() => {
    if (!matches || matches.length === 0) return [];
    
    const managerNames = matches
      .map(match => match['AHLY MANAGER'])
      .filter(manager => manager && manager.toString().trim() !== '')
      .map(manager => manager.toString().trim());
    
    return [...new Set(managerNames)].sort();
  }, [matches]);

  // Filter matches based on selected manager and other filters
  const filteredMatches = useMemo(() => {
    ...
    return matches.filter(match => {
      ...
      if (filterRef && (!match['REFREE'] || match['REFREE'].toString().trim() !== filterRef)) return false;
  ...
  }, [storeUniqueValues, matches, selectedManager, filterSyscom, filterChampion, filterSeason, filterRef, filterManagerOpp, filterHAN, filterStad, filterTeamOpp, filterWDL, dateFrom, dateTo]);

  // ============= دوال خاصة بإحصائيات المدربين =============
  
  /**
   * إنشاء Index للأهداف المسجلة من PLAYERDETAILS مرة واحدة
   * يجعل البحث O(1) بدلاً من O(n)
   */
  const goalsIndex = useMemo(() => {
    ...
    playerDetailsData.forEach(row => {
      ...
      const rowGA = row['GA'] ? row['GA'].toString().trim() : '';
      const rowTeam = row['TEAM'] ? row['TEAM'].toString().trim() : '';
      
      if (rowGA === 'GOAL' && rowMatchId && rowTeam) {
  ...
  /**
   * حساب عدد الأهداف للفريق الهوم (AHLY TEAM) - محسّن
   */
  const calculateHomeGoals = useCallback((matchId, homeTeam) => {
    if (!matchId || !homeTeam) return 0;
    const key = `${matchId}|${homeTeam}`;
    return goalsIndex.get(key) || 0;
  }, [goalsIndex]);

  /**
   * حساب عدد الأهداف للفريق الأواي (OPPONENT TEAM) - محسّن
   */
  const calculateAwayGoals = useCallback((matchId, awayTeam) => {
  ...
  /**
   * حساب Clean Sheet للفريق الهوم (AHLY TEAM)
   */
  const calculateHomeCleanSheet = useCallback((match) => {
    const ga = match['GA'] ? parseInt(match['GA']) : 0;
    return ga === 0 ? 1 : 0;
  }, []);

  /**
   * حساب Clean Sheet للفريق الأواي (OPPONENT TEAM)
   */
  const calculateAwayCleanSheet = useCallback((match) => {
    const gf = match['GF'] ? parseInt(match['GF']) : 0;
    return gf === 0 ? 1 : 0;
  }, []);
  ...
  const competitionStats = useMemo(() => {
    ...
    filteredMatches.forEach(match => {
      ...
      // حساب الأهداف للفريق الهوم والأواي
      if (matchId && homeTeam) {
        statsMap[competition].goalsHome += calculateHomeGoals(matchId, homeTeam);
        statsMap[competition].cleanSheetsHome += calculateHomeCleanSheet(match);
      }
      if (matchId && awayTeam) {
        statsMap[competition].goalsAway += calculateAwayGoals(matchId, awayTeam);
        statsMap[competition].cleanSheetsAway += calculateAwayCleanSheet(match);
      }
    });
    
    return Object.values(statsMap).sort((a, b) => b.matches - a.matches);
  }, [filteredMatches, calculateHomeGoals, calculateAwayGoals, calculateHomeCleanSheet, calculateAwayCleanSheet]);
  ...
  const seasonStats = useMemo(() => {
    ...
    filteredMatches.forEach(match => {
      ...
      // حساب الأهداف والكلين شيت للفريق الهوم والأواي
      if (matchId && homeTeam) {
        statsMap[season].goalsHome += calculateHomeGoals(matchId, homeTeam);
        statsMap[season].cleanSheetsHome += calculateHomeCleanSheet(match);
      }
      if (matchId && awayTeam) {
        statsMap[season].goalsAway += calculateAwayGoals(matchId, awayTeam);
        statsMap[season].cleanSheetsAway += calculateAwayCleanSheet(match);
      }
    });
  ...
  const opponentStats = useMemo(() => {
    ...
    filteredMatches.forEach(match => {
      ...
      // حساب الأهداف والكلين شيت للفريق الهوم والأواي
      if (matchId && homeTeam) {
        statsMap[opponent].goalsHome += calculateHomeGoals(matchId, homeTeam);
        statsMap[opponent].cleanSheetsHome += calculateHomeCleanSheet(match);
      }
      if (matchId && awayTeam) {
        statsMap[opponent].goalsAway += calculateAwayGoals(matchId, awayTeam);
        statsMap[opponent].cleanSheetsAway += calculateAwayCleanSheet(match);
      }
    });
  ...
  // Calculate statistics by opponent managers
  const opponentManagerStats = useMemo(() => {
    if (!filteredMatches || filteredMatches.length === 0) return [];
    
    const statsMap = {};
    
    filteredMatches.forEach(match => {
      const opponentManager = match['OPPONENT MANAGER'] || 'Unknown';
      const matchId = match['MATCH_ID'] ? match['MATCH_ID'].toString().trim() : '';
      const homeTeam = match['AHLY TEAM'] ? match['AHLY TEAM'].toString().trim() : '';
      const awayTeam = match['OPPONENT TEAM'] ? match['OPPONENT TEAM'].toString().trim() : '';
      
      if (!statsMap[opponentManager]) {
        statsMap[opponentManager] = {
          opponentManager,
          matches: 0,
          wins: 0,
          draws: 0,
      ...
      statsMap[opponentManager].matches++;
      
      const wdl = match['W-D-L'] ? match['W-D-L'].toString().trim() : '';
      if (wdl === 'W') statsMap[opponentManager].wins++;
      else if (wdl === 'D' || wdl === 'D.') statsMap[opponentManager].draws++;
      else if (wdl === 'L') statsMap[opponentManager].losses++;
      
      // حساب الأهداف والكلين شيت للفريق الهوم والأواي
      if (matchId && homeTeam) {
        statsMap[opponentManager].goalsHome += calculateHomeGoals(matchId, homeTeam);
        statsMap[opponentManager].cleanSheetsHome += calculateHomeCleanSheet(match);
      }
      if (matchId && awayTeam) {
        statsMap[opponentManager].goalsAway += calculateAwayGoals(matchId, awayTeam);
        statsMap[opponentManager].cleanSheetsAway += calculateAwayCleanSheet(match);
      }
    });
            ...
            <table className="stats-table">
              <thead>
                <tr>
                  ...
                  <th>Goals Home</th>
                  <th>Goals Away</th>
                  <th>Clean Sheets Home</th>
                  <th>Clean Sheets Away</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <>
                    {competitionStats.map((stat, index) => (
                      <tr key={index}>
                        ...
                        <td className="stat-cell">{stat.goalsHome}</td>
                        <td className="stat-cell">{stat.goalsAway}</td>
                        <td className="stat-cell">{stat.cleanSheetsHome}</td>
                        <td className="stat-cell">{stat.cleanSheetsAway}</td>
                      </tr>
                    ))}
                    <tr className="total-row">
                      ...
                      <td className="stat-cell" style={{fontWeight: 700}}>{competitionStats.reduce((sum, stat) => sum + stat.goalsHome, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{competitionStats.reduce((sum, stat) => sum + stat.goalsAway, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{competitionStats.reduce((sum, stat) => sum + stat.cleanSheetsHome, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{competitionStats.reduce((sum, stat) => sum + stat.cleanSheetsAway, 0)}</td>
                    </tr>
                  </>
                ) : (
                  <tr>
                ...
                {seasonStats.length > 0 ? (
                  <>
                    ...
                    <tr className="total-row">
                      ...
                      <td className="stat-cell" style={{fontWeight: 700}}>{seasonStats.reduce((sum, stat) => sum + stat.goalsHome, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{seasonStats.reduce((sum, stat) => sum + stat.goalsAway, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{seasonStats.reduce((sum, stat) => sum + stat.cleanSheetsHome, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{seasonStats.reduce((sum, stat) => sum + stat.cleanSheetsAway, 0)}</td>
                    </tr>
                  </>
                ) : (
                  <tr>
                ...
                {opponentStats.length > 0 ? (
                  <>
                    ...
                    <tr className="total-row">
                      ...
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentStats.reduce((sum, stat) => sum + stat.goalsHome, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentStats.reduce((sum, stat) => sum + stat.goalsAway, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentStats.reduce((sum, stat) => sum + stat.cleanSheetsHome, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentStats.reduce((sum, stat) => sum + stat.cleanSheetsAway, 0)}</td>
                    </tr>
                  </>
                ) : (
                  <tr>
        ...
        return (
          <div className="stats-table-container">
            <table className="stats-table">
              ...
              <tbody>
                {opponentManagerStats.length > 0 ? (
                  <>
                    {opponentManagerStats.map((stat, index) => (
                      <tr key={index}>
                        <td className="opponent-cell">{stat.opponentManager}</td>
                        <td className="stat-cell">{stat.matches}</td>
                        <td className="stat-cell">{stat.wins}</td>
                        <td className="stat-cell">{stat.draws}</td>
                        <td className="stat-cell">{stat.losses}</td>
                    ...
                    <tr className="total-row">
                      <td className="opponent-cell" style={{fontWeight: 700}}>Total</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentManagerStats.reduce((sum, stat) => sum + stat.matches, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentManagerStats.reduce((sum, stat) => sum + stat.wins, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentManagerStats.reduce((sum, stat) => sum + stat.draws, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentManagerStats.reduce((sum, stat) => sum + stat.losses, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentManagerStats.reduce((sum, stat) => sum + stat.goalsHome, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentManagerStats.reduce((sum, stat) => sum + stat.goalsAway, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentManagerStats.reduce((sum, stat) => sum + stat.cleanSheetsHome, 0)}</td>
                      <td className="stat-cell" style={{fontWeight: 700}}>{opponentManagerStats.reduce((sum, stat) => sum + stat.cleanSheetsAway, 0)}</td>
                    </tr>
                  </>
                ) : (
                  <tr>
                  ...
                  filteredMatches.map((match, index) => {
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td className="stat-cell">{calculateHomeGoals(matchId, homeTeam)}</td>
                        <td className="stat-cell">{calculateAwayGoals(matchId, awayTeam)}</td>
                        <td className="stat-cell">{calculateHomeCleanSheet(match) ? 'Yes' : 'No'}</td>
                        <td className="stat-cell">{calculateAwayCleanSheet(match) ? 'Yes' : 'No'}</td>
                      </tr>
                    );
                  })
                ) : (
    ...
    return (
      <div className="manager-stats-page">
        <div className="page-header">
          <h1 className="page-title">Manager Statistics</h1>
        </div>
        <div className="loading">Loading manager data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="manager-stats-page">
      ...
      <div className="search-container">
        <div className="manager-dropdown-wrapper">
          <SearchableDropdown
            options={allManagers}
      ...
      <div className="tabs-container">
        ...
        <button
          className={`tab-button ${activeTab === 'opponent-managers' ? 'active' : ''}`}
          onClick={() => setActiveTab('opponent-managers')}
        >
          Opponent Managers
        </button>
        <button
      ...
      <div className="stats-content">
        {selectedManager && (
          <div className="manager-info">
            ...
            <h2>{selectedManager}</h2>
            <p>{filteredMatches.length} matches managed</p>
          </div>
        )}
