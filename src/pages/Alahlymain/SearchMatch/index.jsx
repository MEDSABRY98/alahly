...
export default function SearchMatch() {
  ...
  // Function to extract base minute for sorting (handles 45+2' -> 45)
  const getBaseMinute = (minuteStr) => {
    if (!minuteStr) return 0;
    const minute = minuteStr.toString();
    const baseMinute = minute.split('+')[0]; // Get part before '+'
    return parseInt(baseMinute) || 0;
  };

  // Function to sort goals by minute
  const sortGoalsByMinute = (goals) => {
    return [...goals].sort((a, b) => {
      const minuteA = getBaseMinute(a.minute);
      const minuteB = getBaseMinute(b.minute);
  ...
  return (
    <div className="search-match">
      ...
      {/* Match Details Section - Football App Style */}
      {matchFound && (
        <div className="match-details-container">
          {/* Match Header */}
          <div className="match-header">
            {/* Match Basic Info Row */}
            <div className="match-basic-info">
              <div className="basic-info-item">
                <span className="basic-info-label">التاريخ</span>
                <span className="basic-info-value">{matchData.basic.date || 'غير محدد'}</span>
              </div>
              <div className="basic-info-item">
                <span className="basic-info-label">الموسم</span>
                <span className="basic-info-value">{matchData.basic.season || 'غير محدد'}</span>
              </div>
              <div className="basic-info-item">
                <span className="basic-info-label">الحكم</span>
                <span className="basic-info-value">{matchData.basic.ref || 'غير محدد'}</span>
              </div>
            </div>
            
            <div className="match-teams">
              <div className="team-info home-team">
                <h2 className="team-name">{matchData.basic.team1 || 'الأهلي'}</h2>
                <div className="coach-name">{matchData.basic.manager || ''}</div>
              </div>
              
              <div className="match-score">
                <div className="score-display">
                  <span className="score">{matchData.basic.gf || 0}</span>
                  <span className="score-separator">-</span>
                  <span className="score">{matchData.basic.ga || 0}</span>
                </div>
              </div>
              
              <div className="team-info away-team">
                <h2 className="team-name">{matchData.basic.teamOpp || 'الخصم'}</h2>
                <div className="coach-name">{matchData.basic.managerOpp || ''}</div>
              </div>
            </div>
          </div>
          ...
          {/* Goals Section */}
          {matchData.goals.goals.length > 0 && (
            <div className="goals-section">
              <h3 className="section-title">الأهداف</h3>
              <div className="goals-container">
                {/* Home Team Goals */}
                <div className="team-goals home-goals">
                  <div className="team-goals-header">
                    <div className="team-logo-small">الأهلي</div>
                    <div className="team-name-small">الأهلي</div>
                  </div>
                  <div className="goals-list">
                    {sortGoalsByMinute(matchData.goals.goals
                      .filter(goal => goal.team === 'AHLY' || goal.team === 'الأهلي'))
                      .map((goal, index) => (
                        <div key={index} className="goal-item home-goal">
                          <div className="goal-content">
                            <div className="goal-player">
                              {goal.playerName}
                              {goal.type && goal.type.toLowerCase().includes('pen') && (
                                <span className="pen-indicator"> (PENGOAL)</span>
                              )}
                              {goal.goalAssist && goal.goalAssist.trim() && (
                                <div className="goal-assist-inline">
                                  <span className="assist-icon">⚽</span>
                                  {goal.goalAssist}
                                  {goal.goalAssistType && goal.goalAssistType.toLowerCase().includes('pen') && (
                                    <span className="pen-indicator"> (PENASSIST)</span>
                                  )}
                                </div>
                              )}
                            </div>
                          ...
                          <div className="goal-minute">{goal.minute}'</div>
                        </div>
                      ))}
                  </div>
                </div>
                ...
                {/* Away Team Goals */}
                <div className="team-goals away-goals">
                  <div className="team-goals-header">
                    <div className="team-logo-small">{matchData.basic.teamOpp?.charAt(0) || 'ض'}</div>
                    <div className="team-name-small">{matchData.basic.teamOpp || 'الخصم'}</div>
                  </div>
                  <div className="goals-list">
                    {sortGoalsByMinute(matchData.goals.goals
                      .filter(goal => goal.team !== 'AHLY' && goal.team !== 'الأهلي'))
                      .map((goal, index) => (
                        <div key={index} className="goal-item away-goal">
                          <div className="goal-content">
                            <div className="goal-player">
                              {goal.playerName}
                              {goal.type && goal.type.toLowerCase().includes('pen') && (
          ...
          {/* Goalkeepers Section */}
          {matchData.goalkeepers.goalkeepers.length > 0 && (
            <div className="goalkeepers-section">
              <h3 className="section-title">حراس المرمى</h3>
              <div className="goalkeepers-list">
                {matchData.goalkeepers.goalkeepers.map((gk, index) => (
                  <div key={index} className={`goalkeeper-item ${gk.team === 'AHLY' ? 'home-gk' : 'away-gk'}`}>
                    <div className="gk-name">{gk.playerName}</div>
                    <div className="gk-stats">
                      <span>الأهداف المستقبلة: {gk.goalsConceded}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="match-actions">
            ...
            <button 
              ...
              disabled={saving}
            >
              {saving ? 'جاري التحديث...' : 'تحديث الماتش'}
            </button>
          </div>
        </div>
      )}
